...
namespace KatsukiBot.Commands.Twitch.Handler {
    public static class TwitchCommandUtils {

        internal static async Task<ArgumentBindingResult> BindArguments(TwitchCommandContext ctx, bool ignoreSurplus) {
            ...
            for (var i = 0; i < overload.Arguments.Count; i++) {
                ...
                if (arg.IsCatchAll && arg.IsArray) {
                    ...
                    while (i < rawArgumentList.Count) {
                        try {
                            array.SetValue(await ctx.TwitchCommands.ConvertArgument(rawArgumentList[i], ctx, arg.Type).ConfigureAwait(false), i - start);
                        } catch (Exception ex) {
                    ...
                    try {
                        args[i + 2] = rawArgumentList[i] != null ? await ctx.TwitchCommands.ConvertArgument(rawArgumentList[i], ctx, arg.Type).ConfigureAwait(false) : arg.DefaultValue;
                    } catch (Exception ex) {
                        return new ArgumentBindingResult(ex);
                    }
                }
        ...
        public static int GetStringPrefixLength(this ChatMessage msg, string str, StringComparison comparisonType = StringComparison.Ordinal) {
        ...
        internal static bool IsModuleCandidateType(this TypeInfo ti) {
            ...
            var tmodule = typeof(TwitchBaseCommandModule);
        ...
        internal static bool IsCommandCandidate(this MethodInfo method, out ParameterInfo[] parameters) {
            ...
            if (!parameters.Any() || parameters.First().ParameterType != typeof(TwitchCommandContext) || method.ReturnType != typeof(Task))
                return false;
